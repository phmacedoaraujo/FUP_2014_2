...
{
	if(d.tamanho == d.comprimento) return false;
	for(int i=0;i<d.comprimento;i++)
		if(d.vetor[i] == x) return false;
	d.vetor[d.comprimento] = x;
	d.comprimento++;
	return true;
}
//CE
...
{
    for(int i=0;i<d.comprimento;i++) {
	if(d.vetor[i].numero == x) {
		d.vetor[i].frequencia++;
		return true;
	}
    }
    if(d.tamanho == d.comprimento) return false;
